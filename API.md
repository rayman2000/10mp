...
**Request:**
```bash
curl -X POST http://localhost:3001/api/kiosk/register \
...
```json
{
  ...
  "token": "abc123xyz789def4",
  "status": "pending",
  "message": "Kiosk registered successfully. Waiting for admin activation."
}
```
...
**Notes:**
- `token`: 16-character cryptographically secure token (generated by kiosk)
- `kioskId`: Unique identifier for the kiosk device
- `kioskName`: Optional display name for admin console
- Kiosk status starts as `pending` until admin activates it

---

### GET /api/kiosk/status/:token

Poll for kiosk activation status. Kiosk calls this every 2 seconds while waiting.
...
**Request:**
```bash
curl http://localhost:3001/api/kiosk/status/abc123xyz789def4
```
...
**Notes:**
- Updates `lastHeartbeat` timestamp on each call
- Returns `sessionId` when kiosk is activated
- Kiosk frontend polls this endpoint every 2 seconds
...
Activate a pending kiosk (admin only).
...
**Request:**
```bash
curl -X POST http://localhost:3001/api/admin/activate-kiosk \
  -H "Content-Type: application/json" \
  -d '{
    "token": "abc123xyz789def4",
    "sessionId": "main-game"
...
**Notes:**
- Admin must manually verify kiosk before activating
- Associates kiosk with a game session
...
List kiosks by status (admin only).

**Query Parameters:**
- `status` (string, optional): Filter by status (`pending`, `active`, `inactive`, `all`)
  - Default: `all`

**Request:**
```bash
# Get all pending kiosks
curl http://localhost:3001/api/admin/pending-kiosks?status=pending

# Get all kiosks
curl http://localhost:3001/api/admin/pending-kiosks
```
  ...
  "kiosks": [
    {
      ...
      "activatedAt": null,
      "lastHeartbeat": "2025-01-26T12:05:00.000Z"
    },
    {
      "id": "uuid-2",
      ...
      "lastHeartbeat": "2025-01-26T12:10:00.000Z"
    }
  ]
}
```
...
**Notes:**
- Returns kiosks filtered by status
- `lastHeartbeat` shows when kiosk last polled for status
- Use to monitor active kiosks and approve pending ones
